...
generateBtn.addEventListener("click", async () => {
  ...
  output.innerHTML = `<div class="flex items-center justify-center space-x-2">
  ...
  try {
    ...
    output.innerHTML = `
      ...
      <p>${result.output}</p>
      ${result.bullet_points?.length ? `<ul class="list-disc ml-5">${result.bullet_points.map(b => `<li>${b}</li>`).join("")}</ul>` : ""}
      ${result.extra_notes ? `<p class="text-gray-600 italic">${result.extra_notes}</p>` : ""}
    `;
  } catch (err) {
